...
export default class ParticipantEventsService {
  ...
  static async accomplishedEventByActivityId(activityId: ObjectId): Promise<IResponse> {
    let updateResult;
    try {
      updateResult = await ParticipantEventModel.updateOne({"activityId": activityId}, {"$set": {status: StatusEventsType.ACCOMPLISHED}});
    } catch (e) {
      throw new InternalServerErrorResponse(e);
    }
